...
const PropertyCard = ({ speciesData, data, pokemonTypeData }: { speciesData: any, data: any, pokemonTypeData: any }) => {
        ...
        <Row className="show-grid">
          ...
          <Col xs={12} sm={12} lg={6} xl={6}>
            <div className='flex-col'>
              ...
              {speciesData?.egg_groups?.length && (speciesData?.egg_groups?.map((item: any, index: number) => (<span key={item.name} className='prop-header-data'>{getCamleCaseString(item.name)}
        ...
        <Row className="show-grid pt-3">
          <Col xs={12} sm={12} lg={6} xl={6}>
            <div className='flex-col'>
              ...
              {data.abilities.length && (data.abilities.map((item: any, index: number) => (<span key={item.ability.name} className='prop-header-data'>{getCamleCaseString(item.ability.name)}
            ...
            <div className='flex-col'>
              ...
              <div className='prop-header-data'>
                <div className='type-wrap'>
                  {data.types.length && (data.types.map((item: any, index: number) => (
          ...
          <Col xs={12} sm={12} lg={12} xl={12}>
            <div className='flex-col'>
              ...
              <div className='prop-header-data type-wrap'>
                {pokemonTypeData.damage_relations.double_damage_from.length && (pokemonTypeData.damage_relations.double_damage_from.map((item: any, index: number) => (
